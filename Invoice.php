...
class Invoice{
	...
	private $details = 'invoice_details';
	private $info = 'invoice_info';
	...
	public function saveInvoice($POST) {
		...
		for ($i = 0; $i < count($POST['productCode']); $i++) {
			$sqlInsertItem = "
			INSERT INTO ".$this->invoiceOrderItemTable."(order_id,caseNo,hsnCode,item_code,item_name, order_item_quantity,peices,netWt,gWt,curr,order_item_price,netAmt,CGST,SGST,IGST,taxAmt,order_item_final_amount) 
			VALUES ('".$lastInsertId."', '".$POST['caseNo'][$i]."','".$POST['hsnCode'][$i]."','".$POST['productCode'][$i]."', '".$POST['productName'][$i]."', '".$POST['quantity'][$i]."','".$POST['peices'][$i]."','".$POST['netWt'][$i]."','".$POST['gWt'][$i]."','".$POST['curr'][$i]."','".$POST['price'][$i]."','".$POST['netAmt'][$i]."','".$POST['CGST'][$i]."','".$POST['SGST'][$i]."','".$POST['IGST'][$i]."','".$POST['taxAmt'][$i]."','".$POST['total'][$i]."')";			
			mysqli_query($this->dbConnect, $sqlInsertItem);
		} 
		      	
		for ($i = 0; $i < count($POST['pod']); $i++) {
			$sqlInsertItem = "
			INSERT INTO ".$this->details."(order_id,coo,cod,pcb,por,pol,pod) 
			VALUES ('".$lastInsertId."','".$POST['coo'][$i]."', '".$POST['cod'][$i]."', '".$POST['pcb'][$i]."', '".$POST['por'][$i]."', '".$POST['pol'][$i]."', '".$POST['pod'][$i]."')";	
			mysqli_query($this->dbConnect, $sqlInsertItem);
			}
		for ($i = 0; $i < count($POST['tgw']); $i++) {
			$sqlInsertItem = "
			INSERT INTO ".$this->info."(order_id,cn,ct,sm,tp,tnw,tgw) 
			VALUES ('".$lastInsertId."','".$POST['cn'][$i]."', '".$POST['ct'][$i]."', '".$POST['sm'][$i]."', '".$POST['tp'][$i]."', '".$POST['tnw'][$i]."', '".$POST['tgw'][$i]."')";	
			mysqli_query($this->dbConnect, $sqlInsertItem);
			}	 
	}	
	...
	public function getInvoiceItems2($invoiceId){
		$sqlQuery = "
			SELECT * FROM ".$this->info." 
			WHERE order_id = '$invoiceId'";
		return  $this->getData($sqlQuery);	
	}	
	public function getInvoiceItems1($invoiceId){
		$sqlQuery = "
			SELECT * FROM ".$this->details."
			WHERE order_id = '$invoiceId'";
		return  $this->getData($sqlQuery);	
	}
